...
use super::inventory_write_service::{
    ...
    InventoryWriteService, NewReceipt, SubmitOutcome,
};
...
impl InventoryWriteService {
    ...
    pub async fn submit_purchase_receipt_into(
        ...
        // posts nothing (stays `not_applicable`); a `periodic` valuation policy suppresses
        // the real-time post the same way (the closing flow — a later increment — owns those legs).
    ...
    pub async fn repost_purchase_receipt(&self, id: Uuid, sink: &dyn GlPostSink) -> Result<SubmitOutcome, InventoryError> {
        ...
        // valuation policy suppresses the real-time post — the unsettled leg retires to
