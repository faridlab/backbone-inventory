//! Domain policy for Product aggregate
//!
...
/// Domain policy for Product — permits all operations (no business invariants).
///
/// To add domain rules, replace this alias with a struct implementing
/// `backbone_core::DomainPolicy<Product>` in the `// <<< CUSTOM` zone.
pub type ProductDomainPolicy = PermitAllPolicy<Product>;
