//! Domain policy for OperationType aggregate
//!
...
/// Domain policy for OperationType — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<OperationType>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type OperationTypeDomainPolicy = PermitAllPolicy<OperationType>;
