//! Domain policy for LandedCostLine aggregate
//!
...
/// Domain policy for LandedCostLine — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<LandedCostLine>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type LandedCostLineDomainPolicy = PermitAllPolicy<LandedCostLine>;
