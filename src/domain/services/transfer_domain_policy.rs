//! Domain policy for Transfer aggregate
//!
...
/// Domain policy for Transfer — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<Transfer>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type TransferDomainPolicy = PermitAllPolicy<Transfer>;
