//! Domain policy for Stock aggregate
//!
...
/// Domain policy for Stock — permits all operations (no business invariants).
///
/// To add domain rules, replace this alias with a struct implementing
/// `backbone_core::DomainPolicy<Stock>` in the `// <<< CUSTOM` zone.
pub type StockDomainPolicy = PermitAllPolicy<Stock>;
