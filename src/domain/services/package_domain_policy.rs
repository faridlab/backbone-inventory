//! Domain policy for Package aggregate
//!
...
/// Domain policy for Package — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<Package>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type PackageDomainPolicy = PermitAllPolicy<Package>;
