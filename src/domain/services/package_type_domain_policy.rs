//! Domain policy for PackageType aggregate
//!
...
/// Domain policy for PackageType — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<PackageType>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type PackageTypeDomainPolicy = PermitAllPolicy<PackageType>;
