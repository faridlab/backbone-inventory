//! Domain policy for StockMove aggregate
//!
...
/// Domain policy for StockMove — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<StockMove>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type StockMoveDomainPolicy = PermitAllPolicy<StockMove>;
