//! Domain policy for PickingBatch aggregate
//!
...
/// Domain policy for PickingBatch — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<PickingBatch>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type PickingBatchDomainPolicy = PermitAllPolicy<PickingBatch>;
