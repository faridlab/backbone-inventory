//! Domain policy for RouteRule aggregate
//!
...
/// Domain policy for RouteRule — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<RouteRule>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type RouteRuleDomainPolicy = PermitAllPolicy<RouteRule>;
