//! Domain policy for StockMoveLine aggregate
//!
...
/// Domain policy for StockMoveLine — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<StockMoveLine>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type StockMoveLineDomainPolicy = PermitAllPolicy<StockMoveLine>;
