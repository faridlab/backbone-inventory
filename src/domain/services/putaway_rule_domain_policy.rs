//! Domain policy for PutawayRule aggregate
//!
...
/// Domain policy for PutawayRule — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<PutawayRule>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type PutawayRuleDomainPolicy = PermitAllPolicy<PutawayRule>;
