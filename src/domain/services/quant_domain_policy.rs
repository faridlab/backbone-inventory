//! Domain policy for Quant aggregate
//!
...
/// Domain policy for Quant — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<Quant>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type QuantDomainPolicy = PermitAllPolicy<Quant>;
