...
//!
//! **Per-company handler** (the same shape as every backbone scheduled job): the host
//! enumerates its companies and calls this once per company, so the job itself never runs a
//! cross-tenant read (ADR-0008) — `app.company_id` is bound on each batch connection and the
//! RLS fence stays meaningful inside the job.
//!
...
/// Run the daily scheduler for one company. See the module docs for the posture declarations;
...
pub async fn run_scheduler(
    ...
    run_scheduler_with(pool, service, pipeline, company_id, SchedulerBatching::default()).await
}
...
pub async fn run_scheduler_with(
    ...
    company_scope::with_company_scope(Some(company_id), async move {
        ...
        for _ in 0..batching.max_batches {
            ...
            ProcurementRepository::bind_company(&mut tx).await?;
            let claimed =
                ProcurementRepository::claim_orderpoints(&mut *tx, company_id, batching.batch_size).await?;
        ...
        for _ in 0..batching.max_batches {
            ...
            ProcurementRepository::bind_company(&mut tx).await?;
            let moves =
                ProcurementRepository::claim_assignable_moves(&mut *tx, company_id, batching.batch_size).await?;
            ...
            for m in &moves {
                match service.assign_one(&mut tx, pipeline.as_ref(), company_id, m.id).await {
        ...
        for _ in 0..batching.max_batches {
            let mut tx = pool.begin().await?;
            ProcurementRepository::bind_company(&mut tx).await?;
            let removed = service.housekeep_quants(&mut tx, company_id, batching.batch_size).await?;
            tx.commit().await?;
