...
//!
//! Coverage (L1–L14 + the receipt-seam probe):
...
//! - L12 a `standard` costing posture is refused loudly.
...
//! - L14 the GL leg rides posting_state: deferred validate arms `pending` in-tx, repost
...
//!
//! The valuation settings table is a singleton from the bare module's point of view (the
//! composing service's decorator scopes it per org unit): the probes serialize and clear it
//! up front, and every probe assumes the runtime-default posture unless it seeds one.
...
async fn item(w: &InventoryWriteService, pool: &PgPool, weight: Option<&str>) -> Uuid {
    let item_id = Uuid::new_v4();
    w.create_stock_item(backbone_inventory::application::service::inventory_write_service::NewStockItem {
        item_id, stock_uom: "unit".into(),
        valuation_method: None, reorder_level: Decimal::ZERO,
    }).await.unwrap();
    if let Some(wt) = weight {
        sqlx::query("UPDATE inventory.stock_items SET weight_per_unit=$1 WHERE item_id=$2")
            .bind(d(wt)).bind(item_id).execute(pool).await.unwrap();
    }
    item_id
}
...
async fn receive(
    w: &InventoryWriteService, wh: Uuid, a: &Accts,
...
async fn draft_lc(
    w: &InventoryWriteService, receipt: Uuid,
...
/// The valuation settings table is a singleton from the bare module's point of view (the
/// composing service's decorator scopes it per org unit): the probes serialize on this mutex
/// and clear the table up front, and every probe assumes the runtime-default posture unless
/// it seeds one.
static SETTINGS_SERIAL: tokio::sync::Mutex<()> = tokio::sync::Mutex::const_new(());

/// The posting posture (cost-method axis only matters here).
async fn set_cost_method(pool: &PgPool, cost_method: &str) {
    ...
    sqlx::query(
        r#"INSERT INTO inventory.inventory_company_settings
           ...
           VALUES ($1,$2::inventory_cost_method,'perpetual',false)"#,
    )
    .bind(Uuid::new_v4()).bind(cost_method).execute(pool).await.unwrap();
}
...
async fn two_line_receipt(
    w: &InventoryWriteService, pool: &PgPool, a: &Accts, rec: &Recorder,
) -> (Uuid, Uuid, Uuid) {
    let wh = warehouse(w).await;
    let ia = item(w, pool, None).await;
    let ib = item(w, pool, None).await;
    let rid = receive(w, wh, a, &[(ia, "10", "100"), (ib, "2", "250")], rec).await;
    (rid, ia, ib)
}
...
async fn l1_split_by_quantity() {
    let _serial = SETTINGS_SERIAL.lock().await;
    let pool = pool().await;
    sqlx::query("DELETE FROM inventory.inventory_company_settings").execute(&pool).await.unwrap();
    let w = InventoryWriteService::new(pool.clone());
    let rec = Recorder::default();
    let a = accts();
    let (rid, ia, ib) = two_line_receipt(&w, &pool, &a, &rec).await;
    let lc = draft_lc(&w, rid, &[("freight", "quantity", "300")], a.cost).await;
    ...
    assert_eq!(bin(&pool, ia, wh).await, (d("10"), d("125.000000"), d("1250.00")),
        "L1: bin A revalued 1000 + 250");
    assert_eq!(bin(&pool, ib, wh).await, (d("2"), d("275.000000"), d("550.00")),
...
async fn l2_split_by_value() {
    ...
    let a = accts();
    let (rid, ia, ib) = two_line_receipt(&w, &pool, &a, &rec).await;
    let lc = draft_lc(&w, rid, &[("insurance", "value", "300")], a.cost).await;
...
async fn l3_split_by_weight() {
    ...
    let a = accts();
    let wh = warehouse(&w).await;
    let ia = item(&w, &pool, Some("3")).await;
    let ib = item(&w, &pool, Some("0.5")).await;
    let rid = receive(&w, wh, &a, &[(ia, "10", "100"), (ib, "2", "250")], &rec).await;
    let lc = draft_lc(&w, rid, &[("freight", "weight", "310")], a.cost).await;
    w.validate_landed_cost(lc, &rec).await.unwrap();
    let rows = worksheet(&pool, lc).await;
...
async fn l4_zero_denominator_is_loud_with_no_fallback() {
    ...
    let wh = warehouse(&w).await;
    let ia = item(&w, &pool, None).await; // weight 0 — the unweighted default
    let rid = receive(&w, wh, &a, &[(ia, "10", "100")], &rec).await;
    let lc = draft_lc(&w, rid, &[("freight", "weight", "100")], a.cost).await;
    ...
    let ib = item(&w, &pool, None).await;
    let rid0 = receive(&w, wh, &a, &[(ib, "5", "0")], &rec).await;
    let lc0 = draft_lc(&w, rid0, &[("fee", "value", "100")], a.cost).await;
    ...
    let lcz = draft_lc(&w, ridz, &[("freight", "quantity", "100")], a.cost).await;
...
async fn l5_half_up_and_last_line_eats_the_diff() {
    ...
    let a = accts();
    let wh = warehouse(&w).await;
    let items = [item(&w, &pool, None).await, item(&w, &pool, None).await];

    // (a) 1.00 over three equal-qty lines: 0.33, 0.33, and the LAST (by move_line_id) eats 0.34.
    let r3 = receive(&w, wh, &a,
        &[(items[0], "1", "10"), (items[1], "1", "10"), (item(&w, &pool, None).await, "1", "10")], &rec).await;
    let lc3 = draft_lc(&w, r3, &[("fee", "quantity", "1.00")], a.cost).await;
    ...
    let r2 = receive(&w, wh, &a, &[(items[0], "1", "10"), (items[1], "1", "10")], &rec).await;
    let lc2 = draft_lc(&w, r2, &[("fee", "quantity", "0.05")], a.cost).await;
...
async fn l6_worksheet_rebuild_is_destructive_and_deterministic() {
    ...
    let rec = Recorder::default();
    let a = accts();
    let (rid, ia, _ib) = two_line_receipt(&w, &pool, &a, &rec).await;
    let lc = draft_lc(&w, rid,
    ...
    for (mlid, bogus) in [("00000000-0000-0000-0000-000000000001".parse::<Uuid>().unwrap(), d("999")),
        ...
        sqlx::query(
            r#"INSERT INTO inventory.landed_cost_adjustment_lines
                 (id, lc_id, move_line_id, cost_line_id, share, additional_landed_cost, remaining_qty)
               VALUES ($1,$2,$3,$4,$5,$5,0)"#,
        )
        .bind(Uuid::new_v4()).bind(lc).bind(mlid)
...
async fn l7_only_the_remaining_share_revalues_no_cogs_true_up() {
    ...
    let a = accts();
    let wh = warehouse(&w).await;
    let i = item(&w, &pool, None).await;
    // Receive 10 @ 100, then deliver 4 BEFORE the landed cost lands.
    let rid = receive(&w, wh, &a, &[(i, "10", "100")], &rec).await;
    ...
    assert_eq!(bin(&pool, i, wh).await, (d("6"), d("100.000000"), d("600.00")),
        "L7: 6 on hand at the old average before the landed cost");

    let lc = draft_lc(&w, rid, &[("freight", "quantity", "100")], a.cost).await;
    let lc_rec = Recorder::default();
    w.validate_landed_cost(lc, &lc_rec).await.unwrap();
    ...
    assert_eq!(bin(&pool, i, wh).await, (d("6"), d("110.000000"), d("660.00")),
    ...
    ).bind(lc).fetch_optional(&pool).await.unwrap();
...
async fn l8_negative_landed_cost_swaps_legs_and_lowers_the_rate() {
    ...
    let pos = draft_lc(&w, rid, &[("freight", "quantity", "100")], a.cost).await;
    w.validate_landed_cost(pos, &rec).await.unwrap();
    assert_eq!(bin(&pool, i, wh).await, (d("10"), d("110.000000"), d("1100.00")),
        "L8: +100 revalues the full on-hand estate");

    let neg = draft_lc(&w, rid, &[("freight correction", "quantity", "-100")], a.cost).await;
    let neg_rec = Recorder::default();
    w.validate_landed_cost(neg, &neg_rec).await.unwrap();
    assert_eq!(bin(&pool, i, wh).await, (d("10"), d("100.000000"), d("1000.00")),
...
async fn l9_done_cannot_cancel_and_cancel_only_from_draft() {
    ...
    let rec = Recorder::default();
    let a = accts();
    let (rid, _, _) = two_line_receipt(&w, &pool, &a, &rec).await;

    // A draft cancels cleanly.
    let draft = draft_lc(&w, rid, &[("fee", "quantity", "10")], a.cost).await;
    ...
    let done = draft_lc(&w, rid, &[("fee", "quantity", "10")], a.cost).await;
...
async fn l12_standard_cost_method_refuses_loudly() {
    ...
    let rec = Recorder::default();
    let a = accts();
    set_cost_method(&pool, "standard").await;
    let (rid, _, _) = two_line_receipt(&w, &pool, &a, &rec).await;
    let lc = draft_lc(&w, rid, &[("fee", "quantity", "100")], a.cost).await;
    let err = w.validate_landed_cost(lc, &rec).await.unwrap_err();
...
// --- L14: the GL leg rides posting_state ---------------------------------------------------------------

#[tokio::test]
async fn l14_gl_rides_posting_state_pending_then_repost_heals() {
    let _serial = SETTINGS_SERIAL.lock().await;
    let pool = pool().await;
    sqlx::query("DELETE FROM inventory.inventory_company_settings").execute(&pool).await.unwrap();
    let w = InventoryWriteService::new(pool.clone());
    ...
    let a = accts();
    let (rid, _, _) = two_line_receipt(&w, &pool, &a, &rec).await;
    let lc = draft_lc(&w, rid, &[("fee", "quantity", "150")], a.cost).await;
...
async fn is_landed_costs_line_mints_no_stock_and_adds_no_value() {
    ...
    let a = accts();
    let wh = warehouse(&w).await;
    let stock_item = item(&w, &pool, None).await;
    let svc_item = item(&w, &pool, None).await;
    let receipt_number = uq("PR");
    let rid = w.create_purchase_receipt(NewReceipt {
        receipt_number: receipt_number.clone(), branch_id: None, supplier_id: Uuid::new_v4(),
        source_po_id: None, warehouse_id: wh, posting_date: day(), currency: "IDR".into(),
        inventory_account_id: a.inv, grir_account_id: a.grir,
        lines: vec![
    ...
    // Only the stock line minted a move. Moves carry the receipt number as their
    // origin, so the count is scoped to THIS receipt (the scratch DB is shared
    // and accumulates rows across suites).
    let moves: i64 = sqlx::query_scalar(
        "SELECT COUNT(*) FROM inventory.stock_moves WHERE origin = $1")
        .bind(&receipt_number)
        .fetch_one(&pool).await.unwrap();
    ...
    assert_eq!(bin(&pool, stock_item, wh).await, (d("2"), d("50.000000"), d("100.00")),
        "SEAM: the stock line valued normally");
    let svc_bin: i64 = sqlx::query_scalar(
        "SELECT COUNT(*) FROM inventory.bins WHERE item_id=$1")
        .bind(svc_item).fetch_one(&pool).await.unwrap();
